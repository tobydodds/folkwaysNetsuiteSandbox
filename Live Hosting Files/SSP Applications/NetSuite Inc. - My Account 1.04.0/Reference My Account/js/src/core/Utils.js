...
(function ()
{
	...
	//This method will return a new date object if the passed in string is in the following format:
	// YYYY-MM-dd BY DEFAULT. Through a format property in options it is possible to specify in which format the string is
	function stringToDate (str_date, options)
	{
		...
		//plumbing
		,	date_parts = str_date ? str_date.split('-') : []
		...
		,	result = new Date(Date.UTC(year, month,	day) + offset);
