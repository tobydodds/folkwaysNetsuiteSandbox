...
(function ()
{
	...
	function getKeyMapping (application)
	{
		...
		,	_minimumQuantity: function (item)
			{
				...
				if (childs && childs.length && childs[0].get('minimumquantity'))
				{
					return childs[0].get('minimumquantity');
				}
				// Otherways return its own minimumquantity
		...
		,	_isGridy: function (item)
			{
				return !!(item.get('_horizontalGridOption') && item.get('_verticalGridOption'));
			}

		,	_horizontalGridOption: function (item)
			{
				var posible_options = item.getPosibleOptions()
				,	vertical_grid_options = application.getConfig('gridOptions.horizontal') || []
				,	found = null
				,	i = 0;

				while (!found && i < vertical_grid_options.length)
				{
					found = _.findWhere(posible_options, {cartOptionId: vertical_grid_options[i]});
					i++;
				}
		...
		,	_verticalGridOption: function (item)
			{
				var posible_options = item.getPosibleOptions()
				,	vertical_grid_options = application.getConfig('gridOptions.vertical') || []
				,	found = null
				,	i = 0;
